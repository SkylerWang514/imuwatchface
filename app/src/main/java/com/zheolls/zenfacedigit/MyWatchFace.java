...
public class MyWatchFace extends CanvasWatchFaceService {
    ...
    private static final String version = "2.2.0-HIT";
...
//                            .show();
        ...
        public void onDraw(Canvas canvas, Rect bounds) {
            ...
            // Util.secondDatetime: "yyyy-MM-dd_HH:mm:ss"
            String timeText = Util.secondDatetime(System.currentTimeMillis());
            // Draw Date
            ...
            // update notification
            if (connectFlag && LogService.accelerometerListener != null) {
                StringBuilder stringBuilder = null;
                float[] values = null;

                // 加速度传感器数据绘制
                stringBuilder = new StringBuilder();
                stringBuilder.append("Accelerations: ");
                values = LogService.accelerometerListener.getLatestData();
                for (float value : values) {
                    stringBuilder.append(String.format("%.02f", value)).append(" ");
                }
                canvas.drawText(stringBuilder.toString(), mXOffset, mYOffset + 30, mTextAcc);

                // 陀螺仪传感器数据绘制
                stringBuilder = new StringBuilder();
                stringBuilder.append("Gyroscope: ");
                values = LogService.gyroscopeListener.getLatestData();
                for (float value : values) {
                    stringBuilder.append(String.format("%.02f", value)).append(" ");
                }
                canvas.drawText(stringBuilder.toString(), mXOffset, mYOffset + 45, mTextAcc);

                canvas.drawText("Last Save time:" +
                                Util.timeOnlyDatetime(LogService.accelerometerListener.getLastSaveTime()),
                        mXOffset, mYOffset + 75, mTextAcc);
                canvas.drawText(
                        String.format("Buffer: %.2f", LogService.accelerometerListener.getBufferUsedPercentage()),
                        mXOffset, mYOffset + 90, mTextAcc);
            }
        }
